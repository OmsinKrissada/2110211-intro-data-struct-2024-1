...
class list {
   ...
   public:
    ...
    void reorder(int pos, std::vector<int> selected) {
        ...
        auto look_it = selected.begin();
        for (int i = 0; look_it != selected.end() || pos_node == nullptr; i++) {
            if (i == *look_it) {
                look_it++;
        ...
        for (int i = 0; i < selected_nodes.size(); i++) {
            node* current = selected_nodes[i];
            current->prev->next = current->next;
            current->next->prev = current->prev;
            if (i != 0) current->prev = selected_nodes[i - 1];
            if (i != selected.size() - 1) current->next = selected_nodes[i + 1];
        }
        selected_nodes.front()->prev = pos_node->prev;
        pos_node->prev->next = selected_nodes.front();
        selected_nodes.back()->next = pos_node;
        pos_node->prev = selected_nodes.back();
    }
};
