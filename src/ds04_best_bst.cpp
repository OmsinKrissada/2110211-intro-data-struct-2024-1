...
void add(CP::map_bst<int, int>& bst, std::vector<int>::iterator begin, std::vector<int>::iterator end) {
    int size = end - begin;
    if (size == 1) {
        bst[*begin] = 1;
        return;
    ...
    int mid = size / 2;
    bst[*(begin + mid)] = 1;
    add(bst, begin, begin + mid);
    add(bst, begin + mid + 1, end);
}
